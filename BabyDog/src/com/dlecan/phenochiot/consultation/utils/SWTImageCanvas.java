...
/******************************************************************************
 * Copyright (c) 2004 Chengdong Li : cdli@ccs.uky.edu All rights reserved. This
...
/**
 * A scrollable image canvas that extends org.eclipse.swt.graphics.Canvas. <p/>
 * It requires Eclipse (version >= 2.1) on Win32/win32; Linux/gtk;
 * MacOSX/carbon. <p/>This implementation using the pure SWT, no UI AWT package
 * is used. For convenience, I put everything into one class. However, the best
 * way to implement this is to use inheritance to create multiple hierarchies.
 * 
...
public class SWTImageCanvas extends Canvas {
	...
	private void paint(GC gc) {
		...
		if (sourceImage != null) {
			Rectangle imageRect = SWT2Dutil
					.inverseTransformRect(transform, clientRect);
	...
	private void scrollHorizontally(ScrollBar scrollBar) {
		...
		af
				.preConcatenate(AffineTransform
						.getTranslateInstance(select - tx, 0));
		transform = af;
		syncScrollBars();
	}
	...
	private void scrollVertically(ScrollBar scrollBar) {
		...
		double select = -scrollBar.getSelection();
		af
				.preConcatenate(AffineTransform.getTranslateInstance(0, select
																		- ty));
		transform = af;
		syncScrollBars();
	}
	 ...
	 * range, it will correct it. This function considers only following
	...
	public void onFileOpen() {
		...
		fileChooser
				.setFilterNames(new String[]{"SWT image"
	 ...
	 */
	public void centerZoom(	double dx,
							double dy,
							double scale,
							AffineTransform af) {
